...
export function Table() {
  return (
    <table className="w-full border border-gray-600 rounded-md overflow-hidden">
      <thead>
        <tr className="text-left text-gray-300 border-b border-gray-600 bg-slate-800">
      ...
      <tbody>
        {awards.map((item) => (
          ...
          >
            ...
            <td>
              {item.status && (
                <div className="px-2 py-1 inline-block rounded-md border-green-800 bg-green-600 text-white text-xs">
                  Ativo
                </div>
              )}
              {!item.status && (
                <div className="px-2 py-1 inline-block rounded-md border-red-800 bg-red-600 text-white text-xs">
                  Expirado
                </div>
              )}
            </td>
